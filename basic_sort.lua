...
--[[
    ...
    插入排序类似于我们排序扑克牌，每次拿牌堆里的第一张牌，然后把这张牌插入到手上的有序牌堆中的正确位置
    由于手上的牌是有序的，如果拿到的牌比手上最后那张牌都大（或相等），说明不需要再插入
]]
local function insert_sort(arr)
    ...
    for i = 2, len do 
        for j = i, 2, -1 do 
            if arr[j] < arr[j - 1] then
                arr[j], arr[j - 1] = arr[j - 1], arr[j]    --这里既可以理解为类似于冒泡排序的前后两两交换，也可以理解为元素后挪
            else
                break       --如果比已有序的最后一个元素都大，说明改元素不用再插入了
            end
        end
    end
end
