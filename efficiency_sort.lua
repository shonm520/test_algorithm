...
local function heap_sort(arr_input)               --堆排序
    ...
    local function adjust(arr, len, index)        --len的作用仅仅只是限制元素比较的范围 index实际是三元组的顶元素
        local left = 2 * index                    --三元组的左右节点
        local right = left + 1

        local max_index = index                   --假设目前最大元素是顶元素
        ...
        if max_index ~= index then                --如果三元组的最大元素不是他的顶元素 则把目前的顶元素和最大元素交换
            arr[max_index], arr[index] = arr[index], arr[max_index]
            adjust(arr, len, max_index)           --交换之后可能会影响被交换元素为顶组成的三元组的规则 所以再次调整那个三元组 递归下去
        end
    end
